...
const CreateLecturerModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await api.post('/lecturers', submitData);
      
      if (response.data.success) {
        showSuccess('Lecturer created successfully');
        onSuccess();
        onClose();
        resetForm();
      }
      ...
      showError(error.response?.data?.message || 'Failed to create lecturer');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-white dark:bg-dark-800 rounded-lg shadow-xl max-w-4xl w-full max-h-[95vh] sm:max-h-[90vh] overflow-y-auto mx-2 sm:mx-0">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-dark-700">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Add New Lecturer</h2>
              <p className="text-sm text-gray-600 dark:text-gray-400">Create a new lecturer account</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-6">
          ...
          <div className="space-y-4">
            <h3 className="text-lg font-medium text-gray-900 dark:text-white flex items-center space-x-2">
              <HiBadgeCheck className="h-5 w-5 text-brand-600 dark:text-brand-400" />
          ...
          <div className="flex items-center justify-end space-x-4 pt-6 border-t border-gray-200 dark:border-dark-700">
            ...
            >
              ...
              <span>{loading ? 'Creating...' : 'Create Lecturer'}</span>
            </button>
          </div>
        </form>
      </div>
