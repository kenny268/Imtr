...
const CreateFacultyModal = ({ isOpen, onClose, onSuccess }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await api.post('/faculties', formData);
      ...
      if (response.data.success) {
        ...
        showError(response.data.message || 'Failed to create faculty');
      }
    } catch (error) {
      console.error('Error creating faculty:', error);
      showError(error.response?.data?.message || 'Failed to create faculty');
    } finally {
      setLoading(false);
    }
  };
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400">
                Add a new academic faculty to the system
              </p>
            </div>
          </div>
          <button
